...
Current version: **0.11.0** — see [Releases](../../releases) and the [CHANGELOG](CHANGELOG.md).
...
- **Size history and a chart** — every completed scan is recorded rather than overwriting the last reading, so "is this folder growing, or was it always this big?" has an answer. The chart icon beside a folder's name opens a window with day, week, month and quarter ranges; a range the data does not reach yet stays greyed out and says why, so you are never offered an empty plot. Hover the line for the measurement under the pointer. History is plain JSON in Application Support — a manifest plus a file of date/size pairs per folder, readable and editable by hand; the last two days keep every measurement and older points thin to one an hour, so the file stays small. Remove a folder yourself and its history goes with it; a folder that merely went missing — an unmounted drive, say — keeps it, and picks it up again if you re-add the same path. The chart remembers the range you picked, separately for each folder and volume.
- **Disk history** — built-in volumes are recorded too, once an hour, with no setting to fiddle with: free space moves slowly, and closer readings would add nothing but file size. Click a built-in volume's label in the panel to open its chart. Removable drives are not recorded — a drive plugged in for an hour a week draws a line where "the disk filled up" cannot be told apart from "the disk was unplugged" — and which is which is decided by the same signal Finder's eject arrow goes by, not by a mount path. A volume is identified by its filesystem UUID, so its history survives remounting and renaming.

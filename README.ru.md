...
Текущая версия: **0.8.0** — см. [Releases](../../releases) и [CHANGELOG](CHANGELOG.md).
...
- **Извлечение внешних дисков** — кнопка «извлечь» в строке каждого съёмного тома, чтобы отмонтировать диск одним кликом там, где вы на него и смотрите, а не идти за этим в Finder.
- **Формат тома в подсказке диска** — при наведении на том показывать заодно, во что он отформатирован: APFS, HFS+, exFAT, FAT32. Одно слово сразу объясняет, почему свободное место на внешнем диске считается не так, как на загрузочном, и за ответом не надо идти в этот README.
...
- **Убирать всплывающую подсказку при перетаскивании окна** — окошко перетаскивается на новое место, а подсказка, которая была открыта в этот момент (по CPU, памяти или диску), остаётся висеть на старом месте, вместо того чтобы исчезнуть вместе с перемещением.
- **Европейский португальский в приложении** — интерфейс сейчас говорит на бразильском варианте, тогда как этот README написан уже на европейском. В планах — отдельный перевод pt-PT рядом с pt-BR в меню выбора языка, чтобы одно отвечало другому.

...
Текущая версия: **0.10.0** — см. [Releases](../../releases) и [CHANGELOG](CHANGELOG.md).
...
- **История и для дисков** — по папкам размеры теперь записываются и рисуются графиком, а свободное место на дисках по-прежнему измеряется каждые пять секунд и выбрасывается. То же самое ответило бы на вопрос «когда этот диск начал заполняться?»; открытый вопрос — чем опознавать том между переподключениями и переформатированиями, потому что путь монтирования для этого не годится.

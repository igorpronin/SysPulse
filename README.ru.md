...
Текущая версия: **0.11.0** — см. [Releases](../../releases) и [CHANGELOG](CHANGELOG.md).
...
- **История размера и график** — каждый обход теперь записывается, а не затирает предыдущее показание, и на вопрос «папка растёт или всегда была такой?» появился ответ. Иконка графика рядом с именем папки открывает окно с диапазонами за сутки, неделю, месяц и квартал; диапазон, до которого данные ещё не дотягиваются, погашен и говорит почему — пустого графика вам не предложат. Наведение на линию показывает измерение под курсором. История лежит обычным JSON в Application Support: манифест и файл пар «дата — размер» на каждую папку, читаемые и правимые руками; свежие двое суток хранятся как измерялись, глубже — по точке на час, поэтому файл остаётся небольшим. Убрали папку сами — история уходит с ней; папка, которая просто пропала (например, уехала с отмонтированным диском), историю сохраняет и подхватывает её обратно, если добавить тот же путь. Выбранный диапазон запоминается — отдельно для каждой папки и каждого тома.
- **История дисков** — постоянные тома тоже записываются, раз в час, и настраивать тут нечего: свободное место меняется медленно, а более частые измерения добавили бы только веса файлу. График открывается кликом по метке тома в панели. Подключаемые диски не записываются: у диска, который втыкают на час в неделю, по линии не отличить «диск заполнялся» от «диска не было», — а постоянный он или нет, решает тот же признак, по которому Finder рисует стрелку извлечения, а не путь монтирования. Том опознаётся по UUID файловой системы, поэтому история переживает и перемонтирование, и переименование.

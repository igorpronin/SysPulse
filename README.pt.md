...
Versão atual: **0.11.0** — ver [Releases](../../releases) e o [CHANGELOG](CHANGELOG.md).
...
- **Espaço livre em disco** — uma linha por cada volume local montado, com a fração ocupada em barra e o espaço livre em números; passe o ponteiro pela barra para ver quanto está ocupado e o nome completo do volume. Em APFS e HFS+ o valor é exatamente o que o Finder mostra, incluindo o espaço recuperável dos snapshots; em exFAT e FAT — o formato com que a maioria dos discos externos chega — recorre à contagem simples de espaço livre, a única que esses sistemas de ficheiros mantêm. A dica indica também o sistema de ficheiros — APFS, ExFAT, Mac OS Extended —, que foi justamente o que decidiu como aquele espaço livre foi contado. Clicar na barra de um volume abre-o no Finder, e um volume externo ejeta-se com o botão no fim da linha; essa coluna só aparece enquanto houver algo para ejetar. Os volumes que não lhe interessam escondem-se nas definições.
- **Tamanhos de pastas** — acompanhe as pastas que quiser: adicione-as em «Pastas», escrevendo o caminho ou escolhendo-a no Finder, dê a cada uma a sua periodicidade de análise (não atualizar, ou de uma vez por minuto até uma vez por dia), e os tamanhos juntam-se ao painel num bloco próprio, sob um título «Pastas» que traz o total de todas elas. Um botão no título volta a analisar todas de uma vez, e cada pasta tem o seu. Defina um nome alternativo e o painel mostra-o em vez do nome da pasta. Passar o ponteiro por uma pasta indica o caminho, a periodicidade, quando foi medida pela última vez e os dez maiores elementos lá dentro — subpastas e ficheiros soltos ordenados em conjunto, as pastas marcadas com uma barra final —, o suficiente para ver o que está realmente a ocupar o espaço. Clicar no nome de uma pasta abre-a no Finder. Os resultados ficam em cache, por isso o tamanho está no ecrã assim que a aplicação arranca, sem esperar por uma nova análise.
- **Histórico do tamanho e gráfico** — cada análise concluída fica registada em vez de substituir a leitura anterior, e a pergunta «esta pasta está a crescer, ou sempre foi assim?» passa a ter resposta. O ícone de gráfico ao lado do nome da pasta abre uma janela com os intervalos de dia, semana, mês e trimestre; um intervalo que os dados ainda não alcançam fica esbatido e diz porquê, para que nunca lhe seja proposto um gráfico vazio. Passe o ponteiro pela linha para ver a medição sob o cursor. O histórico é JSON simples em Application Support — um manifesto e um ficheiro de pares data/tamanho por pasta, legíveis e editáveis à mão; os últimos dois dias guardam todas as medições e os pontos mais antigos ficam reduzidos a um por hora, por isso o ficheiro mantém-se pequeno. Se for você a remover a pasta, o histórico vai com ela; uma pasta que apenas desapareceu — num disco desmontado, por exemplo — mantém-no, e recupera-o se voltar a acrescentar o mesmo caminho. O gráfico memoriza o intervalo escolhido, separadamente para cada pasta e cada volume.
- **Histórico dos discos** — os volumes fixos também são registados, uma vez por hora, sem nada para configurar: o espaço livre muda devagar, e leituras mais próximas só acrescentariam peso ao ficheiro. O gráfico abre-se clicando na etiqueta do volume no painel. Os discos amovíveis não são registados — num disco ligado uma hora por semana, a linha não distingue «o disco encheu» de «o disco não estava ligado» —, e o que decide entre uns e outros é o mesmo sinal que guia a seta de ejeção do Finder, não o caminho de montagem. Um volume é identificado pelo UUID do seu sistema de ficheiros, por isso o histórico sobrevive a uma nova montagem e a mudanças de nome.
...
- **11 idiomas** — English (predefinição), Русский, Español, Deutsch, Français, Italiano, Português (Brasil), Português (Portugal), 中文, 日本語, 한국어. Trocam-se a partir do menu.
...
- **Ordenar a lista de pastas** — as pastas aparecem no painel pela ordem por que foram adicionadas, e não há outra ordem. Poder escolher entre ordenar por tamanho, por nome ou por ordem de adição deixaria as maiores no topo, que são normalmente a razão para as acompanhar.
